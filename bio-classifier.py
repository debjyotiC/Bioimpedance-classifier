...
data_thp_80 = df_train['80_THP']


data_train = np.array([data_thp_90, data_thp_10], dtype=float)
data_label = np.array([0.0, 1.0], dtype=float)

data_test = np.array([data_thp_80], dtype=float)

model = tf.keras.models.Sequential([
    tf.keras.layers.Dense(100, input_shape=(100, ), activation=tf.nn.tanh),
    tf.keras.layers.Dense(80, activation=tf.nn.relu),
    tf.keras.layers.Dense(50, activation=tf.nn.relu),
    tf.keras.layers.Dense(2, activation=tf.nn.sigmoid)
])

model.compile(loss=tf.losses.BinaryCrossentropy(), optimizer=tf.keras.optimizers.Adam(0.1), metrics=['accuracy'])
